...
async def deliver_free_lesson(message: Message, state: FSMContext, lesson_num: str = "1", delete_previous: bool = False):
    ...
    if not lesson_text:
        if lesson_num == "1":
            lesson_text = (
                "🦷 <b>Секреты правильного прикуса, о которых молчат ортодонты</b>\n\n"
                "Правильный прикус — это не только красивая улыбка, но и здоровье всего организма (суставы, осанка, дыхание). "
                "В этом уроке я разбираю главные ошибки при лечении, показываю неочевидные проблемы "
                "и делюсь планом того, как получить идеальный результат без обмана со стороны клиник!\n\n"
                "🔥 <i>Смотрите видео внимательно до конца, чтобы избежать роковых (и дорогих) врачебных ошибок.</i>"
            )
        elif lesson_num == "2":
            lesson_text = (
                "🪥 <b>Домашняя гигиена, которая сэкономит вам сотни тысяч рублей</b>\n\n"
                "Знаете ли вы, что 90% людей чистят зубы неправильно? Это прямая дорога к кариесу и потере зубов. "
                "В этом уроке я наглядно показываю, как нужно ухаживать за полостью рта каждый день, "
                "чтобы ваши зубы всегда оставались здоровыми.\n\n"
                "💡 <i>Внедрите эти простые советы сегодня, и вам больше никогда не придется бояться бормашины!</i>"
            )
        elif lesson_num == "3":
            lesson_text = (
                "🔩 <b>Вся правда об имплантах: как выбрать и не пожалеть</b>\n\n"
                "Установка импланта — ответственный и дорогой шаг. Как не переплатить и быть уверенным в том, что он приживется? "
                "В этом уроке я разрушаю популярные мифы, объясняю человеческим языком разницу между системами "
                "и даю пошаговое руководство по выбору надежного хирурга.\n\n"
                "🛡 <i>Безопасная имплантация начинается с правильной информации. Включайте видео!</i>"
            )
        else:
            lesson_text = "Приятного просмотра!"
